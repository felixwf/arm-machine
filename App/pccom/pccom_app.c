...
TaskHandle_t PCcom_App_Task_Handler;
...
void PCcom_App_Init(void)
{
    ...
    xReturn = xTaskCreate(PCcom_App_Task, "PCcom_App_Task", PCCOM_STACK_SIZE, NULL, PCCOM_TASK_PRIORITY, &PCcom_App_Task_Handler);
    if(pdPASS != xReturn)
    {
        return;                                      //创建接收任务失败
    }
...
//	u8 Res;
    ...
    else if(USART_GetITStatus(PC_COM, USART_IT_IDLE) != RESET)	// 如果接收到完整的一帧数据
    {
		...
		if(PCcom_App_Task_Handler!=NULL) {
			xTaskNotifyFromISR(PCcom_App_Task_Handler, PC_COM_RX_BUF[2], (eNotifyAction) eSetValueWithOverwrite,&xHigherPriorityTaskWoken);//发送任务通知
			portYIELD_FROM_ISR(PCcom_App_Task_Handler);//如果需要的话进行一次任务切换
		}
